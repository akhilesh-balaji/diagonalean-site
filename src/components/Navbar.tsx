...
function NavLink({ label, href, external }: { label: string; href: string; external: boolean }) {
  return (
    <a
      ...
      className={`text-sm md:text-lg text-base-content px-1.5 md:px-2 py-0.5 rounded-sm transition-all duration-150 hover:text-tertiary hover:italic hover:bg-base-300/50 whitespace-nowrap ${label === "Home" ? "hidden sm:inline" : ""}`}
    >
      {label}
    </a >
  );
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-2 sm:top-2 sm:bottom-auto right-2 z-50 flex items-center justify-center backdrop-blur-lg bg-base-200/80 border-2 border-base-300/80 rounded-lg px-2.5 py-2 text-xs md:text-lg text-warning hover:text-tertiary hover:font-bold hover:bg-base-300/50 transition-all duration-150 cursor-pointer font-bold"
      >
        {dark ? "L" : "D"}
      </button>
      <div className="flex items-center gap-0 md:gap-2 py-2 content-center backdrop-blur-lg  bg-base-200/80 px-4 rounded-xl sm:rounded-xl fixed top-2 border-2  border-base-300/80 z-50 left-1/2 -translate-x-1/2">
